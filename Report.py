...
class View():
    ...
    def date_range(self, text, label_row, label_column, entry_row, entry_column, pady):
        ...
        cal_label.grid(column=label_column, row=label_row, pady=pady, sticky='e')
...
def format_datetime(start_cal, end_cal, start_hr_entry, start_min_entry, start_sec_entry, end_hr_entry, end_min_entry, end_sec_entry):
    ...
    datetime = start_date, end_date, start_time, end_time, start, end
    return datetime

def sales_report(params, comp):
    start_cal, start_hr_entry, start_min_entry, start_sec_entry, end_cal, end_hr_entry, end_min_entry, end_sec_entry = params
    datetime = format_datetime(start_cal, end_cal, start_hr_entry, start_min_entry, start_sec_entry, end_hr_entry, end_min_entry, end_sec_entry)
    start_date, end_date, start_time, end_time, start, end = datetime
...
def get_moves(comp, params, product, location, products, prod_df):
    start_cal, start_hr_entry, start_min_entry, start_sec_entry, end_cal, end_hr_entry, end_min_entry, end_sec_entry = params
    datetime = format_datetime(start_cal, end_cal, start_hr_entry, start_min_entry, start_sec_entry, end_hr_entry, end_min_entry, end_sec_entry)
    start_date, end_date, start_time, end_time, start, end = datetime
    ...
    if product == '':
        ...
        dict = tf.stock_moves(comp, product, location, start_date, end_date, prod_df)
        if dict is None:
            messagebox.showinfo('Info', 'Nothing to display')
        else:
            report_name = f'{product} Movement ({start:%d %b %y} - {end:%d %b %y})'
            doc = rp.Doc(start_date=start_date, end_date=end_date, start_time=start_time, end_time=end_time)
            doc.createDoc([False], [dict], t_title=[f'{product}'])
            doc.createPath(report_name)

def main():
    def create_date_ui(frame):
        start_v = View(frame, start_yr, start_month, start_day)
        end_v = View(frame, end_yr, end_month, end_day)
        
        start_cal = start_v.date_range('From: ', label_row=0, label_column=0, entry_row=0, entry_column=1, pady=10)
        ...
        params = start_cal, start_hr_entry, start_min_entry, start_sec_entry, end_cal, end_hr_entry, end_min_entry, end_sec_entry
        return params
    ...
    def invt_ui():
        ...
        params = categ_listbox, sp_val, wh_val
        return params
    ...
    def moves_ui():
        def search(e):
            value = e.widget.get()
            if value == '':
                search_box['value'] = products
            else:
                data = []
                for i in products:
                    if value.lower() in i.lower():
                        data.append(i)
                search_box['value'] = data

        tk.Label(moves_frame, text='Product:').grid(row=2, column=0, pady=10, sticky='e')
        search_var = tk.StringVar()
        search_box = ttk.Combobox(moves_frame, values=products, textvariable=search_var, font=('Constantia', 9))
        search_box.grid(row=2, column=1, columnspan=3, pady=10, sticky='ew')
        search_box.bind('<KeyRelease>', search)

        location = tk.StringVar()
        location.set('SP/Stock')
        style = ttk.Style().configure('my.TRadiokbutton', font=('Constantia', 6))
        ttk.Radiobutton(moves_frame, text='SP', variable=location, value='SP/Stock', style='my.TRadiobutton').grid(row=2, column=5)
        ttk.Radiobutton(moves_frame, text='WH', variable=location, value='WH/Stock', style='my.TRadiobutton').grid(row=2, column=6)
        ttk.Separator(moves_frame, orient='horizontal').grid(sticky='ew', row=3, columnspan=10)
        return search_var, location
    ...
    moves_frame = ttk.Frame(notebook, width=w, height=h)
    ...
    notebook.add(moves_frame, text='Movement')

    try:
        start_yr, start_month, start_day, start_hour, start_min, start_sec, end_yr, end_month, end_day, end_hour, end_min, end_sec, items, prod_df, comp = tf.get_time()
    except:
    ...
    params = create_date_ui(repo_frame)
    params2 = create_date_ui(moves_frame)
    invt_params = invt_ui()
    search_var, location = moves_ui()

    sales_buttn = View(repo_frame, f_size=9)
    sales_buttn.add_button(func=lambda: sales_report(params, comp), row=6, col=5, colspan=3, padx=10)

    categ_listbox, sp_val, wh_val = invt_params
    ...
    ttk.Button(moves_frame, text='Print', command= lambda: get_moves(comp, params2, search_var.get(), location.get(), products, prod_df)).grid(row=4, column=4, columnspan=3)
